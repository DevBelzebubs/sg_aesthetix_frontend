...
import { AlertCircle, ArrowDown, ArrowLeft, ArrowUp, Folder, Loader2, PencilLine, Plus, Search, Trash2, Undo2, X } from "lucide-react";
...
export default function ServiceCategoriesManagement() {
  ...
  async function saveItem() {
    ...
    try {
      if (mode === "edit" && selectedId !== null) {
        ...
        if (draft.orden !== old) {
          await reorderBeforeInsert(supabase, "categoria_servicio", draft.orden);
        }
        await supabase.from("categoria_servicio").update({
          nombre: draft.nombre,
        ...
        await supabase.from("categoria_servicio").insert({
          nombre: draft.nombre,
          descripcion: draft.descripcion || null,
  ...
  async function moveItem(itemId: number, direction: "up" | "down") {
    ...
    for (let i = 0; i < updated.length; i++) {
      await supabase.from("categoria_servicio").update({ orden: updated[i].orden }).eq("id", updated[i].id);
    }
  }
      ...
      {(mode === "create" || mode === "edit") && (
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--background-secondary)] p-6 shadow-sm">
          <div className="mb-6 flex items-center gap-3">
            <div className="rounded-2xl bg-[var(--background)] p-3"><Folder size={20} className="text-[var(--foreground)]" /></div>
            <div>
              <p className="text-lg font-semibold text-[var(--foreground)]">{mode === "create" ? "Nueva categoría" : "Editar categoría"}</p>
              <p className="text-sm text-[var(--text-muted)]">{mode === "create" ? "Agrega una categoría para servicios." : `Editando ${selectedItem?.nombre ?? ""}`}</p>
            </div>
          </div>
